...
export async function GET(
  ...
  try {
    ...
    if (!invoice) return error("采购发票不存在", 404);
    return success({
      ...invoice,
    ...
    return error(e instanceof Error ? e.message : "获取采购发票详情失败");
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const existing = await prisma.purchaseInvoice.findFirst({ where: { id, companyId } });
    if (!existing) return error("采购发票不存在", 404);
    ...
    return error(e instanceof Error ? e.message : "更新采购发票失败");
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return error(e instanceof Error ? e.message : "删除采购发票失败");
  }
}
